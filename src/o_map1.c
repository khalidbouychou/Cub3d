...
/*   Updated: 2023/11/19 23:42:08 by khbouych         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int check_next_step_2(t_mlx *smlx, float x, float y)
{
	...
	// int oldx = (int)floor(smlx->xplayer / P_SIZE);
	// int oldy = (int)floor(smlx->yplayer / P_SIZE);
	if (x <= 0 || x > smlx->w_window || y <= 0 || y > smlx->h_window)
		return (1);
	// if (smlx->m->sq_map[newy][oldx] == '1' && smlx->m->sq_map[oldy][newx] == '1'
	// 	&& smlx->m->sq_map[newy][oldx] == 'N'
	// 	&& smlx->m->sq_map[oldy][newx] == 'N')
	// 	return (0);
