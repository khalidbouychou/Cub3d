...
/*   Updated: 2023/11/24 17:50:46 by khbouych         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_next_step(t_mlx *smlx, float x, float y)
{
	...
	int oldx = floor(smlx->m->p_x / P_SIZE);
	int oldy = floor(smlx->m->p_y / P_SIZE);
	if (x < 0 || x > smlx->w_window || y < 0 || y > smlx->h_window)
		return (0);
...
void update_pos_player(t_mlx *smlx)
{
	...
	ca = smlx->m->p_x + speed_cos;
	sb = smlx->m->p_y + speed_sin;
...
// 		draw_line(smlx, smlx->m->p_x + (cos(smlx->m->rays[r].rayAngle) * smlx->m->rays[r].distance),
// 							smlx->m->p_y + (sin(smlx->m->rays[r].rayAngle) * smlx->m->rays[r].distance));
// }
...
void move_player(void *param)
{
	...
	// renderRays(smlx);
	draw_player(smlx);
	// draw_line(smlx, smlx->m->p_x + (cos(smlx->m->rotationangle) * 30) ,smlx->m->p_y + (sin(smlx->m->rotationangle) * 30));
}
